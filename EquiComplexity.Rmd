...
Transform data frames from wide format to long format (this is necessary for later plotting and analyses by columns rather than rows). For later comparison we now have one dataframe in which there are still some NAs for certain languages and measures (i.e. Vietnamese and Turkish), and one dataframe where these languages are removed.
```{r}
