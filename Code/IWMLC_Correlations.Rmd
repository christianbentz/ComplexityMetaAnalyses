---
title: "IWMLC2019: Correlations Between Measures"
author: "Chris Bentz"
date: "January 4, 2021"
output:
...
# Load the Data 
The participants' results are loaded as csv files into separate data frames. Only use the name of the first author (lower case) to name the data frame.
```{r}
#Track A
gutierrez <- read.csv("~/Data/IWMLC2019/shared_task/track_A/gutierrez/Gutierrez.csv")
oh <- read.csv("~/Data/IWMLC2019/shared_task/track_A/oh/oh.csv")

#TRACK B
brunato <- read.csv("~/Data/IWMLC2019/shared_task/track_B/brunato_venturi/Brunato-Venturi.csv")
coltekin <- read.csv("~/Data/IWMLC2019/shared_task/track_B/coltekin_rama/coltekin.csv")
semenuks <- read.csv("~/Data/IWMLC2019/shared_task/track_B/semenuks/Semenuks.csv")
sinnemaki <- read.csv("~/Data/IWMLC2019/shared_task/track_B/sinnemaki/Sinnemaki.csv")
sozinova <- read.csv("~/Data/IWMLC2019/shared_task/track_B/sozinova_etal/Sozinova.csv")
```
...
## TRACK A
I here plot correlations between selected measures of the respective track (trying to exclude the ones which are somewhat redundant). The Spearman correlation coefficient is reported instead of the Pearson correlation coefficient. This is because we are only interested whether there is a correlation between the rankings of complexities, regardless of whether this is a linear relationship. I therefore also use the local regression smoothers in the plots (loess) rather than linear models (lm). Note: warning messages are disabled here as there are datasets with NAs, and for each plot this throws a warning message using the ggpairs() plotting function. NAs are delt with by removing the entire row, containing an NA value. The following measures in Gutierrez' contribution are not included here: "GM_TTR.H1", "GM_TTR.H3", "GM_TTR.H1.H3", "GM_TTR.H1_.fullyparallelised.", "GM_TTR.H3_.fullyparallelised.", "GM_TTR.H1.H3_.fullyparallelised.", "GM_H1gram_.fullyparallelised.", "GM_H3gram_.fullyparallelised.", "GM_TTR_.fullyparallelised.". To get a plot of all correlations, the "columns"" argument can just be removed.
...
Safe plot to file.
```{r, fig.width = 7, fig.height = 7, warning = FALSE}
ggsave("/home/chris/Workshops/IWMLC2019/Analyses/track_a_plot.pdf", track.a.plot, dpi = 300, scale = 1, width = 10, height = 10, device = cairo_pdf)
```


## TRACK B
Same for the Track B data. Again, not all of the measures are plotted (there are 27 overall).
...
ggsave("/home/chris/Workshops/IWMLC2019/Analyses/track.a.plot.corrected", track.a.plot.corrected, dpi = 300, scale = 1, width = 10, height = 7.5, device = cairo_pdf)
```
...
```{r, fig.width = 10, fig.height = 7.5, warning = FALSE}
ggsave("/home/chris/Workshops/IWMLC2019/Analyses/track.b.plot.corrected", track.b.plot.corrected, dpi = 300, scale = 1, width = 10, height = 7.5, device = cairo_pdf)
```

## Detailed Plots
The code below adds labels to the points of plots, which helps with the intepretation of results. I here choose the two plots of Track A and Track B with the highest positive Pearson correlations. 
...
track.a.plot1.detailed <- ggplot(track.a, aes(x = GM_TTR_.fullyparallelised., y = O_WID)) + 
  geom_point() +
  geom_smooth(method = loess) +
  geom_label_repel(data = track.a[track.a$language == "Fijian" | track.a$language == "Sango" | track.a$language == "Tagalog" | track.a$language == "Vietnamese" | track.a$language == "Vietnamese" | track.a$language == "Hausa" | track.a$language == "German" | track.a$language == "English" | track.a$language == "Georgian" | track.a$language == "Nama (Namibia)" | track.a$language == "Modern Greek" | track.a$language == "Russian" | track.a$language == "Swahili" | track.a$language == "Basque" | track.a$language == "Finnish" | track.a$language == "Turkish" | track.a$language == "Korean" | track.a$language == "Kalaallisut" | track.a$language == "Burmese" | track.a$language == "Mapudungun", ], 
                   aes(label = language, 
                       fill = factor(language)), color = 'black',
                       size = 3.5) +
  labs(title = paste("r = ", 
                     round(track.a.df[track.a.df$row == "GM_TTR_.fullyparallelised." & track.a.df$col == "O_WID", ]$corr, 2), 
                     sep = "")) +
...
```{r, fig.width = 10, fig.height = 10, warning = FALSE}
ggsave("/home/chris/Workshops/IWMLC2019/Analyses/track.a.plot1.detailed", track.a.plot1.detailed, dpi = 300, scale = 1, width = 10, height = 10, device = cairo_pdf)
```
...
```{r, fig.width = 10, fig.height = 10, warning = FALSE}
ggsave("/home/chris/Workshops/IWMLC2019/Analyses/track.b.plot1.detailed", track.b.plot1.detailed, dpi = 300, scale = 1, width = 10, height = 10, device = cairo_pdf)
```
...
ggsave("/home/chris/Workshops/IWMLC2019/Analyses/track.b.plot.negative.corrected", track.b.plot.negative.corrected, dpi = 300, scale = 1, width = 12, height = 8, device = cairo_pdf)
```
...
```{r, fig.width = 10, fig.height = 10, warning = FALSE}
ggsave("/home/chris/Workshops/IWMLC2019/Analyses/track.b.plot1.negative.detailed", track.b.plot1.negative.detailed, dpi = 300, scale = 1, width = 10, height = 10, device = cairo_pdf)
```
...
```{r, fig.width = 10, fig.height = 10, warning = FALSE}
ggsave("/home/chris/Workshops/IWMLC2019/Analyses/track.b.plot2.negative.detailed", track.b.plot2.negative.detailed, dpi = 300, scale = 1, width = 10, height = 10, device = cairo_pdf)
```
