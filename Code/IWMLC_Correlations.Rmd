...
# Load the Data 
The participants' results are loaded as csv files into separate data frames. We only use the name of the first author (lower case) to name the data frame. Note that the below code to load the data files only works if you have cloned the github repository into a local folder named "Github". The tilde symbol '~' replaces the standard location (e.g. home/chris) in linux systems. This will be different for other operating systems.
```{r}
