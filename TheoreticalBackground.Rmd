...
Firstly, assume that we have a perfect negative Pearson and Spearman correlation between two samples, i.e. $r_{ms} = \rho_{ms} = -1$. For example, assume that m = (1, 2, 3, 4, 5), while the syntactic complexity can be perfectly linearly predicted by $s = 5 -1m$ across five different languages. In other words, the syntactic complexity values are a linear transformation of the morphological complexity values. We thus have the following table of measurements per language and domain:
...
Note that in this particular case, we also have $\overline{m}$ = $\overline{s} = 3$. However, it is of course not necessarily the case that the means for languages and the means of measurements per domain are equal. For the numerator of the Pearson correlation we have
...
Since the syntactic complexity here decreases less then the morphological complexity increases, we now get differing mean values across the languages.
...
example.df <- data.frame(language, m, s_prime, rank_m, rank_s_prime)
print(example.df)
```
...
In the proof above we have used two slightly differing linear transformations to derive $s$ and $s^{\prime}$ from m. A general formular for linear transformations is $y = a + bx$. In the first case we used a = 5 and b = -1, and in the second case a = 5 and b = -0.5. It is noteworthy that standardization of samples neutralizes such linear transformations.    


# Summary
We have thus found a simple transformation of our syntactic complexity values which yields the exact same perfect negative correlations as the untransformed syntactic complexity values for both the Pearson and the Spearman correlation. However, while in the case of the original syntactic complexity values ($s$) the means across languages are also the same, in the scenario of transformed values ($s^{\prime}$), they are not. This proofs that not even perfect negative Pearson and Spearman correlations entail equality of means, i.e.
