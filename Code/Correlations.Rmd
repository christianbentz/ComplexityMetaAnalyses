...
track.a <- track.a[ , -which(names(track.a) %in% c("GM_H1gram", "GM_H3gram", "GM_TTR",
                                                   ...
                                                   "GM_TTR.H1_fullyparallelised", "GM_TTR.H3_fullyparallelised",
                                                   "GM_TTR.H1.H3_fullyparallelised"))]
# Remove measures in Track B
track.b <- track.b[ , -which(names(track.b) %in% c("BV_avg_max_depth","BV_avg_token_per_clause","SI_double_dl", "SI_head_dl", "SI_zero_dl"))]
```
...
```{r}
track.a.scaled <- cbind(track.a[1:2], scale(track.a[3:ncol(track.a)])) # keep meta-information columns again
...
Safe to file.
```{r, warning = FALSE}
ggsave("~/Github/ComplexityMetaAnalyses/Figures/Corrs/scatterplots.pdf", scatterplots, dpi = 300, scale = 1, width = 9, height = 9, device = cairo_pdf)
```
